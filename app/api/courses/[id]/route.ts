...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const course = await prisma.course.findUnique({ where: { courseId: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = await prisma.course.findUnique({ where: { courseId: params.id } })
    ...
    const course = await prisma.course.update({ where: { courseId: params.id }, data: updateData })
    return NextResponse.json(course)
  } catch (error) {
    return serverErrorResponse()
  }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.course.delete({ where: { courseId: params.id } })
